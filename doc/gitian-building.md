...
*Setup instructions for a gitian build of Chronon using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Chronon
...
- [Building Chronon](#building-chronon)
...
Clone the git repositories for chronon and gitian and then checkout the chronon version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the chronon repository
...
To build Chronon (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the chronon repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/chronon/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/robinadaptor/chronon
...
For example:
```bash
URL=https://github.com/robinadaptor/chronon.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit chronon=${COMMIT} --url chronon=${URL} ../chronon/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit chronon=${COMMIT} --url chronon=${URL} ../chronon/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit chronon=${COMMIT} --url chronon=${URL} ../chronon/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/chronon-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/chronon-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/chronon-build.assert
```
...
[chronon/gitian.sigs](https://github.com/roginadaptor/gitian.sigs/) repository, or if that's not possible to create a pull
request.
