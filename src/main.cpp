...
/** Fees smaller than this (in uchn) are considered zero fee (for relaying and mining)
...
const std::string strMessageMagic = "Chronon Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, bool fZerocoinActive, bool fRejectBadUTXO, CValidationState& state)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        //duplicate zcspend serials are checked
...
int64_t GetBlockValue(int nHeight)
{
	...
	if( nHeight > 1 && nHeight <= 3300 ) {
	        ...
	        nSubsidy = 200 * COIN;
	} else if( nHeight > 71600 && nHeight <= 670000 ) {
		nSubsidy = 100 * COIN;
	} else if( nHeight > 670000 && nHeight <= 674999 ) {
		nSubsidy = 100 * COIN;
	} else if( nHeight > 674999 && nHeight <= 675000 ) {
		nSubsidy = 0 * COIN;				
	} else {
		nChkCnt = (nHeight-665000)/5000;
		if(nChkCnt > 5) {
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        if (!Params().IsCrossingChain(pindexNew->nHeight, CChainParams::MUGCUP)) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> chnInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            chnInputs.push_back(stakeIn);
        }
        const bool hasCHNInputs = !chnInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasCHNInputs)
                    ...
                    for (const CTxIn& chnIn : chnInputs){
                        if(chnIn.prevout == in.prevout){
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : chnInputs) {
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Chronon: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
    // SPORK_14 is used for 70714 (v1.6+)
    if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
            return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 was used for 70714 (v1.5+), commented out now.
