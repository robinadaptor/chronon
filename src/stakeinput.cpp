...
//!CHN Stake
bool CChnStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CChnStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CChnStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CChnStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CChnStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CChnStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CChnStake::GetUniqueness()
{
    //The unique identifier for a CHN stake is the outpoint
...
CBlockIndex* CChnStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
