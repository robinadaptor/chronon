...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // Make sure to create the correct block version after blockchain is upgraded
    if(Params().IsCrossingChain(nHeight, CChainParams::MUGCUP)) {
        pblock->nVersion = CChainParams::MUGCUP;
    } else if(Params().IsCrossingChain(nHeight, CChainParams::COFFEE)) {	
        pblock->nVersion = CChainParams::COFFEE;	
    } else if(Params().IsCrossingChain(nHeight, CChainParams::SUGAR)) {	
        pblock->nVersion = CChainParams::SUGAR;				
    } else if(Params().IsCrossingChain(nHeight, CChainParams::PRIMM)) {	
		pblock->nVersion = CChainParams::PRIMM;
    } else {
		pblock->nVersion = CChainParams::PLATE;	
	}
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zCHN spends can have very large priority, use non-overflowing safe functions
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ChrononMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("ChrononMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("ChrononMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ChrononMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadChrononMiner(void* parg)
{
    boost::this_thread::interruption_point();
    CWallet* pwallet = (CWallet*)parg;
    try {
        ChrononMiner(pwallet, false);
        boost::this_thread::interruption_point();
    } catch (std::exception& e) {
        LogPrintf("ThreadChrononMiner() exception");
    } catch (...) {
        LogPrintf("ThreadChrononMiner() exception");
    }

    LogPrintf("ThreadChrononMiner exiting\n");
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThreadChrononMiner, pwallet));
}
