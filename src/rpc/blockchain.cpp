...
void validaterange(const UniValue& params, int& heightStart, int& heightEnd)
{
    if (params.size() < 2) {
...
UniValue getblockindexstats(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
                ...
                "  \"txbytes\": xxxxx                (numeric) Sum of the size of all txes (zCHN excluded) over block range\n"
                "  \"ttlfee\": xxxxx                 (numeric) Sum of the fee amount of all txes (zCHN mints excluded) over block range\n"
                "  \"ttlfee_all\": xxxxx             (numeric) Sum of the fee amount of all txes (zCHN mints included) over block range\n"
