...
bool MultisigDialog::createMultisigTransaction(std::vector<CTxIn> vUserIn, std::vector<CTxOut> vUserOut, std::string& feeStringRet, std::string& errorRet)
{
    try{
        ...
        if(totalIn < totalOut){
            throw std::runtime_error("Not enough CHN provided as input to complete transaction (including fee).");
        }
        ...
        if(tx.vout.at(changeIndex).nValue > fee){
            ...
            throw std::runtime_error("Not enough CHN provided to cover fee");
        }
...
#ifdef ENABLE_WALLET
            // Case 1: Chronon address and we have full public key:
