 ...
 * Chronon system. There are three: the main network on which people trade goods
...
public:
	...
	/** Upgrade Blockchain **/
	bool IsCrossingChain(const int nHeight, int nCode) const
	{
		int nBlockLimit = 0;
		
		if(nCode == CChainParams::MUGCUP) {
		 	nBlockLimit = 735000;
		} else if(nCode == CChainParams::COFFEE) {
		 	nBlockLimit = 730000;
		} else if(nCode == CChainParams::SUGAR)	{
			nBlockLimit = 725000;			
		} else if(nCode == CChainParams::PRIMM) {
			nBlockLimit = 1;
		} else {
			nBlockLimit = 0;		
		}
		
		if(nHeight >= nBlockLimit) return true;
		else return false; 
	}
...
protected:
    ...
    int64_t nRequiredMasternodeCollateral;
