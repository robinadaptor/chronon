...
/**
 * Fees smaller than this (in uchn) are considered zero fee (for transaction creation)
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, int blockHeight)
{
    ...
    if (GetBoolArg("-chnstake", true)) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CChnStake> input(new CChnStake());
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 CHN collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 CHN collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 5000 CHN.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT5000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 5000 CHN.");
                    } else {
...
std::string CWallet::GetUniqueWalletBackupName(bool fzchnAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzchnAuto ? "-autozchnbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}
