...
bool CMasternode::UpdateFromNewBroadcast(CMasternodeBroadcast& mnb)
{
    if (mnb.sigTime > sigTime) {
        ...
        lastTimeChecked = 0;
        int nDoS = 0;
        if (mnb.lastPing == CMasternodePing() || (mnb.lastPing != CMasternodePing() && mnb.lastPing.CheckAndUpdate(nDoS, false, false))) {
            lastPing = mnb.lastPing;
...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!unitTest) {
        ...
        CTxOut vout = CTxOut(Params().GetRequiredMasternodeCollateral() - 0.01 * COIN, obfuScationPool.collateralPubKey);
...
bool CMasternodeBroadcast::Sign(CKey& keyCollateralAddress)
{
    ...
    if(!Params().IsCrossingChain(chainActive.Height(), CChainParams::COFFEE))
