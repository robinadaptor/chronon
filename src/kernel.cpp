...
uint256 ComputeStakeModifier(const CBlockIndex* pindexPrev, const uint256& kernel)
{
    ...
    if (!Params().IsCrossingChain(pindexPrev->nHeight + 1, CChainParams::MUGCUP))
...
bool Stake(const CBlockIndex* pindexPrev, CStakeInput* stakeInput, unsigned int nBits, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    ...
    // iterate the hashing
    bool fSuccess = false;
    const unsigned int nHashDrift = 45;
...
bool initStakeInput(const CBlock block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    CChnStake* chnInput = new CChnStake();
    chnInput->SetInput(txPrev, txin.prevout.n);
    stake = std::unique_ptr<CStakeInput>(chnInput);
    return true;
}
